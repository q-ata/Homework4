"""
The SimpleLang compiler performs extensive rewriting and defining of functions.
The SimpleLang compiler is designed to inspect objects and functions defined by other
frameworks, such as NumPy. The SimpleLang compiler is designed to be extensible, so that
...
SimpleLang compiler tracks properties of attributes/methods of objects or classes.
Properties of the object/class itself are accessed with the `__attr__` property.
...
```python
from autovec import register_property

register_property(complex, "__add__", "is_associative", lambda obj: True)
```

SimpleLang compiler includes a convenience functions to query each property as well,
for example:
```python
from autovec import query_property
...
`element_type` attribute of a tensor is defined with the `__attr__` property, so
that if a user defines a custom tensor class, they can override the `__attr__`
property of the `element_type` attribute by defining a `element_type` in the class
itself.
"""
...
def query_property(obj: type | Hashable, attr: str, prop: str, *args) -> Any:
    ...
    if prop == "__attr__":
        ...
        if isinstance(obj, Hashable) and not isinstance(obj, type):
            msg += (
                f"`autovec.register_property({repr(obj)}, '{attr}', '{prop}', "
                "lambda ...)` or "
            )
        msg += (
            f"`autovec.register_property({obj_name}, '{attr}', '{prop}', lambda ...)`"
        )
    ...
    msg += (
        " See https://github.com/Parallelizing-Compilers/Homework4/algebra/algebra.py "
