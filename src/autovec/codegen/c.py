...
def init_shared_lib(c_code, globals=None, cc=None, cflags=None):
    mod = load_shared_lib(c_code, cc, cflags)
    if globals is not None:
        handles.append(globals)
        for name, value in globals.items():
            setter = getattr(mod, f"set_{name}")
            setter(value)
    return mod
...
# Register serialize_to_c for basic Python types
register_property(int, "serialize_to_c", "__attr__", lambda fmt, obj: ctypes.c_int(obj))
register_property(
    float, "serialize_to_c", "__attr__", lambda fmt, obj: ctypes.c_double(obj)
)

# Register deserialize_from_c for basic Python types (no-op since they're immutable)
register_property(int, "deserialize_from_c", "__attr__", lambda fmt, obj, c_obj: None)
register_property(float, "deserialize_from_c", "__attr__", lambda fmt, obj, c_obj: None)
...
register_property(ctypes.Structure, "c_type", "__attr__", lambda x: x)
...
class CContext(Context):
    ...
    def add_global(self, name, value):
        ...
        t = self.ctype_name(type(value))
        self.add_header(f"{t} {name};\nvoid set_{name}({t} x) {{ {name} = x; }}")
    ...
    def emit_globals(self):
        """
        Emit the global values for the C code.
        """
        return self.globals
...
class CBufferFType(BufferFType, CArgumentFType, ABC):
    ...
    def c_vecgather(self, ctx, buffer, base_offset_idxs, stride):
        """
        Return C code which vector gather a named buffer at the given index.
        """
        ...

    @abstractmethod
    def c_vecscatter(self, ctx, buffer, base_offset_idxs, stride, value):
        """
        Return C code which vector scatter a named buffer to the given index.
        """
        ...
...
def struct_c_type(fmt: StructFType):
    ...
    if res:
        return res
    print("============================")
    for f, t in c_structs.items():
        print(f.struct_name)
        print(f.struct_fields)
        print(t)
    print(fmt.struct_name)
...
register_property(
    ...
    "c_type",
    "__attr__",
    lambda fmt: struct_c_type(fmt),
)


def struct_c_getattr(fmt: StructFType, ctx, obj, attr):
    return f"{obj}.{attr}"
...
def struct_c_setattr(fmt: StructFType, ctx, obj, attr, val):
...
register_property(
    ...
    "construct_from_c",
    "__attr__",
    lambda fmt, c_tuple: tuple(
        getattr(c_tuple, name) for name in fmt.struct_fieldnames
    ),
)
