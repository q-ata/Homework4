...
class SimpleLangNode(Term):
    """
    SimpleLangNode

    Represents a SimpleLang IR node. SimpleLang is the final intermediate
    ...
    def __str__(self):
        ...
        ctx = SimpleLangPrinterContext()
        ctx(self)
        return ctx.emit()


class SimpleLangTree(SimpleLangNode, TermTree):
    @property
    def children(self):
...
class SimpleLangExpression(SimpleLangNode):
    @property
    @abstractmethod
...
class Literal(SimpleLangExpression):
    """
...
@dataclass(eq=True, frozen=True)
class Variable(SimpleLangExpression):
    """
...
@dataclass(eq=True, frozen=True)
class Index(SimpleLangExpression):
    """
...
@dataclass(eq=True, frozen=True)
class VectorIndex(SimpleLangNode):
    """
    Represents a logical AST expression for a vectorized index range.

    Attributes:
        start: The starting index of the vector range (inclusive).
        end: The ending index of the vector range (exclusive).
        stride: The step size between indices in the range.
    """
...
@dataclass(eq=True, frozen=True)
class Call(SimpleLangExpression, SimpleLangTree):
    """
    ...
    def is_vectorized(self):
        for arg in self.args:
            if isinstance(arg, Load | Store) and arg.is_vectorized():
                return True
        return False


@dataclass(eq=True, frozen=True)
class Load(SimpleLangExpression, SimpleLangTree):
    """
    ...
    buffer: Variable
    indices: tuple[
        SimpleLangExpression | VectorIndex, ...
...
class ForLoop(SimpleLangTree):
    ...
    lvl: Index
    start: SimpleLangExpression
    end: SimpleLangExpression
    stride: SimpleLangExpression
    body: Block
...
class SimpleLangPrinterContext(Context):
    ...
    def block(self) -> "SimpleLangPrinterContext":
        blk = super().block()
    ...
    def __call__(self, prgm: SimpleLangNode):
        feed = self.feed
        match prgm:
            case Literal(value):
